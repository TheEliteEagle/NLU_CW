...
def get_dependency_tree_depth(text: str) -> int:
    doc = nlp(text)
    depths = []
    for token in doc:
        depths.append(token.dep)
...
def get_feature_vector(text: str, tensor: torch.Tensor, dense: bool = True, verbose: bool = False) -> np.ndarray:
    ...
    if verbose:
        print(f"Reading Grade: {readability}")

    feat_vec = np.append(feat_vec, readability)

    # # 10. Dependency tree depth
    # dep_tree_depth = get_dependency_tree_depth(text)
    #
    # if verbose:
    #     print(f"Dependency Tree Depth: {dep_tree_depth}")
    #
    # feat_vec = np.append(feat_vec, dep_tree_depth)
...
def main():
    ...
    if create_vecs:
        ...
        col_names = [f"fv_diff_{i}" for i in range(feat_vec_size)] + ["label"]
        ...
        for s1, s2, label in dataloader:
            ...
            s1_s2_diff = s1_fv - s2_fv

            row = np.concatenate((s1_s2_diff, label.numpy()), axis=0)
        ...
        feat_vec_df.to_csv("feature_vectors_pure_diff.csv", index=False)

        feat_vec_df = pd.read_csv("feature_vectors_pure_diff.csv")
    ...
    for i in range(epochs):
        ...
        # if len(last_3) == 3:
        #     last_3.pop(0)
        # last_3.append(np.mean(loss_list))

        if len(last_3) == 3 and (last_3[0] - last_3[1]) < 0.0001 and (last_3[1] - last_3[2]) < 0.0001:
            break
